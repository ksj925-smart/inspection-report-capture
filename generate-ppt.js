...
// ── 이미지 비율 유지 피팅 헬퍼 ──────────────────────────────────
// maxW × maxH 박스 안에 rw:rh 비율 이미지를 중앙 피팅
...
window.generateJointReportPPT = async function({ capturedData, selectedSides, segments }) {
  ...
  for (const side of ['outboard', 'inboard']) {
    ...
    const cols = segs <= 6 ? 3 : 4;   // OUTER/INNER RACE용 기본 cols
